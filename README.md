...
- **Группирует** папки по **приложению** (часть после 3-й запятой) и **породе/товару** (части 4 и 5). Пример: `2260 CH, VP, 40_2023, Add. VP-2508-05, PM (TS), spr 16 pack` → таблица `Add. VP-2508-05, PM (TS), spr`.
- **Обработка «N pack»** — количество упаковок (13 pack, 16 pack и т.п.) не попадает в имя файла и не создаёт отдельные таблицы. Если «N pack» слито с товаром без запятой (`spr 16 pack`), скрипт убирает суффикс и использует `spr`.
- **Для каждой группы:** копирует шаблон, первый лист каждого xlsx-счёта (с форматированием, 25% масштаб, все столбцы видимы), заполняет лист **Total**: колонка B — номера счетов, J — ЭСД, O — декларации GTD.

**Имена файлов** по умолчанию — короткие (без диапазона). В конце скрипт предлагает:
- **1** — закрыть без изменения имён;
- **2** — добавить в имена диапазон и количество (например: `(43;93;95-97;100) 6 pcs., LI, ...`).
...
**Рабочая папка:**
- Папки со счетами (или подпапка `invoices/`) с xlsx-счетами и PDF (ЭСД, GTD).
- Сюда сохраняются готовые отгрузочные таблицы.
